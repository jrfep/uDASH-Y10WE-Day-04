...
The most prestigious award for young statisticians in 2019 is awarded to Hadley Wickham, the author of the ggplot2 package and other great packages, including tidyverse.

![2019 COPSS Presidents' Award: Hadley Wickham](2019_COPSS_Award.png)
...
![Building Layers using ggplot2](ggplot2_Layers.png)

## Dataset

```{r}
# View a summary of the data called "mpg"
...
Note that 'aesthetics' in ggplot refers to WHAT is plotted, not HOW it is plotted, unlike the word's usual meaning. Using the `aes()` function will cause the visual to be based on the *data* specified in the argument. For example, using `aes(colour = "blue")` won’t cause the geometry's colour to be 'blue', but will instead cause the visual to be mapped from the vector `c("blue")` — as if we only had a single class of vehicles that happened to be called 'blue'. If you wish to apply an aesthetic property to an entire geometry, you can set that property as an argument to the geom method, OUTSIDE of the `aes()` call:
